### Web Scraping with R - Exercise Script (with Answer)
...
scraping <- function(x){
  tibble(signatures = html_nodes(x, "td:nth-child(6)") %>% html_text(), # Scraping the petition's title
         mp = html_nodes(x, "td:nth-child(5)") %>% html_text()) # Scraping the number of signatures
}
...
url <- "https://petitions.ourcommons.ca/en/Petition/Search?View=D&parl=43&type=&keyword=&sponsor=&status=&Text=&RPP=20&order=Recent&Page=1&category=All"
